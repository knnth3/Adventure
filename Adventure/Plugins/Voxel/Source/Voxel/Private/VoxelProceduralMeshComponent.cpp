...
void UVoxelProceduralMeshComponent::FinishPhysicsAsyncCook(UBodySetup* FinishedBodySetup)
{
	...
	if (AsyncBodySetupQueue.Find(FinishedBodySetup, FoundIdx))
	{
		...
		// Now update the navigation. (Removed! changed in 4.20 to something different)
		//if (FNavigationSystem::WantsComponentChangeNotifies() && IsRegistered())
		//{
		//	UWorld* MyWorld = GetWorld();

		//	auto NavSystemV1 = (MyWorld) ? UnavigationSystemV1::GetNavigationSystem(GetWorld()) : nullptr;

		//	if (NavSystemV1 != nullptr && (NavSystemV1->ShouldAllowClientSideNavigation() || !MyWorld->IsNetMode(ENetMode::NM_Client)))
		//	{
